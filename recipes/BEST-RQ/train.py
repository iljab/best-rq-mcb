...
class BestRQBrain(sb.core.Brain):
    ...
    def compute_codebook_perplexity(self, targets):
        """Computes the codebook perplexity for the given targets."""
        unique_codes = torch.unique(targets)
        counts = torch.tensor([(targets == code).sum().item() for code in unique_codes], dtype=torch.float32)
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_ste):
        ...
        if (
            ...
            _, targets = outputs
            codebook_perplexity = self.compute_codebook_perplexity(targets)
