...
To run this recipe call python train_multi_codebook_per_cluster.py best_rq.yaml --find_unused_parameters --max_grad_norm 0.0
...
class BestRQBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        for c in range(num_codebooks):
            ...
            # Compute the loss for all samples in the batch without reduction, to weight manually
            loss = F.cross_entropy(cb_pred, cb_target.squeeze(), reduction='none')
            weighted_loss = loss * weights[c]
            total_loss += weighted_loss.sum() / (B * T)

        loss = total_loss / num_codebooks
        return loss
