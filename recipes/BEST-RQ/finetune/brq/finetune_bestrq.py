...
> python finetune_bestrq.py finetune_bestrq_codebook.yaml
...
def dataio_prepare(hparams):
    ...
    if hparams["sorting"] == "ascending":
        ...
        train_data = train_data.filtered_sorted(
            sort_key="duration",
            key_max_value={"duration": hparams["avoid_if_longer_than_seconds"]},
            key_min_value={"duration": hparams["avoid_if_shorter_than_seconds"]},
        )
    ...
    def audio_pipeline(wav):
        if hparams["padding_type"] == "none":
            sig = sb.dataio.dataio.read_audio(wav)
            return sig

        sig, sample_rate = torchaudio.load(wav)
        duration_seconds = sig.shape[1] / sample_rate
        if duration_seconds < hparams["min_seconds_padding"]:
            num_padding_samples = int(sample_rate * (hparams["min_seconds_padding"] - duration_seconds))

            if hparams["padding_type"] == "zero":
                ### zero padding
                padding = torch.zeros((sig.shape[0], num_padding_samples))
                sig = torch.cat((sig, padding), dim=1)
            elif hparams["padding_type"] == "noise":
                ### noise padding
                noise = torch.randn((sig.shape[0], num_padding_samples)) * 0.001
                sig = torch.cat((sig, noise), dim=1)
            elif hparams["padding_type"] == "double_noise":
                ### noise padding (double sided)
                num_padding_samples = int(num_padding_samples // 2)
                noise_s = torch.randn((sig.shape[0], num_padding_samples)) * 0.001
                noise_e = torch.randn((sig.shape[0], num_padding_samples)) * 0.001
                sig = torch.cat((noise_s, sig, noise_e), dim=1)
            elif hparams["padding_type"] == "smooth":
                ### smooth padding
                padding = torch.linspace(1, 0, num_padding_samples).unsqueeze(0).repeat(sig.shape[0], 1)
                sig = torch.cat((sig, sig[:, -1:] * padding), dim=1)
    ...
    label_encoder.expect_len(hparams["output_neurons_ctc"])
