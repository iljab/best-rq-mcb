...
To run this recipe call python train_kl_div.py best_rq.yaml --find_unused_parameters --max_grad_norm 0.0
    ...
    def compute_objectives(self, outputs, batch, stage):
        ...
        if self.hparams.loss_use_ce:
            ...
            for c in range(num_codebooks):
                ce_losses.append(F.cross_entropy(pred_list[c], targets[c].squeeze()))
            ce_loss = (sum(ce_losses) / num_codebooks) * self.hparams.loss_ce_weight
            losses.append(ce_loss)

        if self.hparams.loss_use_kl_div:
            if self.hparams.loss_kl_div_single_distribution:
                combined_pred = torch.cat(pred_list, dim=-1)
                combined_dist = torch.cat(distances, dim=-1)
                kl_loss = F.kl_div(
                    F.log_softmax(combined_pred, dim=-1),
                    F.softmax(combined_dist, dim=-1),
                    reduction="batchmean"
                )
            else:
                kl_losses = []
