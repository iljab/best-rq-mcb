"""This lobe enables the integration of fairseq pretrained hubert models.
...
try:
    ...
    MSG = "Please install Fairseq to use pretrained hubert\n"
...
class FairseqHubert(nn.Module):
    ...
    ---------
    pretrained_path : str
        Path of the pretrained hubert model. It can be a url or a local path.
    save_path : str
    ...
    dropout : float (default: None)
        ...
        dropout rates. This is useful if the hubert model has been trained
    ...
    layer_drop : float (default: None)
        If different from None (0.0 to 1.0), it will override the given fairseq
        layer_drop rate. This is useful if the hubert model has been trained
    ...
    >>> save_path = "models_checkpoints/hubert.pt"
    >>> model = FairseqHubert(model_url, save_path)
    ...
    def __init__(
        ...
        # Download the pretrained hubert model. It can be local or online.
        ...
        # hubert pretrained models may need the input waveform to be normalized
        ...
        if self.freeze:
            logger.warning(
                "speechbrain.lobes.models.fairseq_hubert - hubert is frozen."
            )
            self.model.eval()
            # Freeze parameters
            ...
            if self.freeze_feature_extractor:
                logger.warning(
                    "speechbrain.lobes.models.fairseq_hubert - hubert feature extractor is frozen."
                )
    ...
    def forward(self, wav, wav_lens):
        """Takes an input waveform and return its corresponding hubert encoding.
    ...
    def extract_features(self, wav, padding_mask=None):
        """Extracts the hubert embeddings"""
        # We normalize the input signal if needed.
        if self.normalize:
            wav = F.layer_norm(wav, wav.shape[1:])

        # Extract hubert output
